...
export default function GroupDetailPage({ params }: GroupDetailPageProps) {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [fetchedGroup, fetchedBeneficiaries, fetchedPayments] =
        await Promise.all([
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-primary">
              {formatKes(group.currentBalanceKes)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {group.beneficiaryCount.toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Total funds disbursed to beneficiaries
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <DetailRow
            label="Krona Ratio Weight"
            value={`${group.kronaRatio.toLocaleString()} KR`}
            Icon={Percent}
          />
          <DetailRow
            label="Description"
